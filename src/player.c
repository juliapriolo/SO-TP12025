...
static int find_self_index(GameState* st) {
    ...
    for (unsigned i = 0; i < st->player_count && i < 9; ++i) {
        if (st->players[i].pid == me) { idx = (int)i; break; }
    }
    return idx;
}
...
static int pick_direction(GameState* st, unsigned short x, unsigned short y) {
    for (int dir = 0; dir < 8; ++dir) {
        ...
        if (cell > 0) return dir; // libre: recompensa 1..9
    }
    return -1;
}
...
int main(int argc, char* argv[]) {
    ...
    if (!state) { perror("shm_connect(/game_state)"); return 1; }
    GameSync*  sync  = (GameSync*) shm_connect("/game_sync",  sizeof(GameSync), O_RDWR);
    if (!sync) { perror("shm_connect(/game_sync)"); shm_unmap(state, state_bytes); return 1; }
    ...
    while (!fin) {
        if (sem_wait(&sync->sem_player_can_send[self]) == -1) { perror("sem_wait(G[i])"); break; }
        ...
        int dir = -1;
        if (!fin) dir = pick_direction(state, x, y);
        leave_reader(sync);

        if (fin) break;

        unsigned char move = (dir >= 0) ? (unsigned char)dir : 0; // si no hay libres, intentamos 0 (probablemente inválido)
        ssize_t wr = write(1, &move, 1); // pipe anónimo: máster asocia fd=1 al extremo de escritura
        if (wr != 1) { perror("write(move)"); break; }
